...
const pageNowName = ref('Lista de roles')
const prevPageUrl = ref('')


// TABLAS --------------------------
let endpoint = ref(`v1/app-roles/collection`)

const getList = ref([])
const tableHeaders = ['ID', 'Nombre', 'Permiso', 'Fecha de registro', 'Opciones'];
const tableTitle = ref('Lista de roles')
...
const actions = [
    ...
    },
    {
        ...
        class_icon: 'mdi mdi-server-security',
        tooltip: 'Permisos'
    },
]
...
function onEdit(data) {
    ...
    ids = btoa(ids);
    router.visit(`/admin/roles/${ids}/editar`);
}
...
function onAdd(data) {
    router.visit(`/admin/roles/nuevo`);
}

function onPermissions(data) {
    let ids = data.id.toString();
    ids = btoa(ids);
    router.visit(`/admin/roles/${ids}/asignar`);
}
    ...
    alert
        ...
        .then((result) => {
            if (result.value == 'Confirmar') {
                api
                    .delete(`v1/app-roles/${data.id}/destroy`)
                    .then((response) => {
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <table-pagination :headers="tableHeaders" :tbody="tbody" :options="true" :actions="actions"
                        ...
                        :labelBtnNew="'Nuevo rol'" :showBtnNew="true" :reload="reloadPage" @reload="reload"/>
                </div>
            </div>
        </div>
    </div>
